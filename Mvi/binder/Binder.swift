...
//
//    func bind<T>(_ observer: AnyPublisher<T, Never>, to: Consumer, name: String? = nil) {
//        bind(observer, to: to, using: { $0 }, name: name)
//    }
//
//    func bind<Out, In>(_ observer: AnyPublisher<Out, Never>, to: Consumer, using transform: @escaping (Out) -> In?, name: String? = nil) {
//        bind(observer, to: to, using: connector(from: transform))
//    }
//
//    func bind<Out, In>(_ observer: AnyPublisher<Out, Never>, to: Consumer, using connector: @escaping Connector<Out, In>, name: String? = nil) {
